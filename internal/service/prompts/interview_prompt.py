...
GREETING_PROMPT = ChatPromptTemplate.from_messages([
    (
        ...
        "   - After responding, set `go_to_next_step = true`\n\n"
...
INTRO_PROMPT = ChatPromptTemplate.from_messages([
    (
        ...
        "   - Ask one of the allowed questions from EXAMPLE QUESTIONS, set `go_to_next_step = false`\\n"
        ...
        "   - Stay in the INTRO step: `go_to_next_step = false`\\n\\n"

        "4. **If the candidate provided a valid introduction response:**\\n"
        "   - Set `message = \\\"\\\"`, `go_to_next_step = true`\\n"
        "   - The system will automatically move to the next enabled stage based on user's selected stages\\n\\n"
...
ASK_EXPERIENCE_PROMPT = ChatPromptTemplate.from_messages([
    (
        ...
        "- **ABSOLUTE MAXIMUM: 15 questions total in experience section** (across ALL companies)\n"
        "- If candidate has ONLY 1 company: **Maximum 7 questions total**\n"
        "- If candidate has 2+ companies: **Maximum 7 questions per company** (but never exceed 15 total)\n"
        "- These are HARD LIMITS - once reached, you MUST transition to ASK_PROJECT immediately\n\n"
        ...
        "**STEP LOGIC (MUST FOLLOW EXACTLY):**\n"
        "1. **Count TOTAL experience questions asked so far**:\n"
        "   - Look at conversation history\n"
        "   - Count EVERY question you (AI) asked in the ASK_EXPERIENCE state\n"
        "   - Include ALL questions about ANY company experience\n\n"

        "2. **Check resume_info to identify companies**:\n"
        "   - Count how many companies are listed\n"
        "   - Identify company names, roles, dates, tools, responsibilities\n\n"

        "3. **Apply HARD LIMITS (check before asking anything)**:\n"
        "   - If total experience questions >= 15 → **STOP - transition to ASK_PROJECT**\n"
        "   - If only 1 company AND total questions >= 7 → **STOP - transition to ASK_PROJECT**\n"
        "   - If current company has been asked 7 questions → Move to next company (if exists)\n"
        "   - If all companies covered OR limits reached → **STOP - transition to ASK_PROJECT**\n\n"

        "4. **DUPLICATE CHECK (before selecting a question)**:\n"
        "   - Review ALL questions already asked in conversation history\n"
        "   - Do NOT ask:\n"
        ...
        "5. **If you can still ask (limits not reached)**:\n"
        "   - Select a unique question from EXAMPLE QUESTIONS that hasn't been asked\n"
        "   - Make sure it's relevant to the current company\n"
        "   - Set `go_to_next_step = false`\n\n"

        "6. **If transitioning to next step (limits reached or no more questions)**:\n"
        "   - Set `message = \"\"`, `go_to_next_step = true`\n\n"
...
ASK_PROJECT_PROMPT = ChatPromptTemplate.from_messages([
    (
        ...
        "**HARD LIMITS (MUST OBEY):**\n"
        "- **ABSOLUTE MAXIMUM: 15 questions total in project section** (across ALL projects)\n"
        "- If candidate has ONLY 1 project: **Maximum 7 questions total**\n"
        "- If candidate has 2+ projects: **Maximum 7 questions per project** (but never exceed 15 total)\n"
        "- These are HARD LIMITS - once reached, you MUST transition to TECHNICAL_QUESTION immediately\n"
        "- **FLEXIBILITY:** You don't need to ask all questions if the project has been sufficiently explored.\n\n"
        ...
        "**STEP LOGIC (MUST FOLLOW EXACTLY):**\n"
        "1. **Count TOTAL project questions asked so far**:\n"
        "   - Look at conversation history\n"
        "   - Count EVERY question you (AI) asked in the ASK_PROJECT state\n"
        "   - Include ALL questions about ANY project\n\n"

        "2. **Check resume_info to identify projects**:\n"
        "   - Count how many projects are listed\n"
        "   - Identify project names, technologies, descriptions\n\n"

        "3. **Apply HARD LIMITS (check before asking anything)**:\n"
        "   - If total project questions >= 15 → **STOP - transition to TECHNICAL_QUESTION**\n"
        "   - If only 1 project AND total questions >= 7 → **STOP - transition to TECHNICAL_QUESTION**\n"
        "   - If current project has been asked 7 questions → Move to next project (if exists)\n"
        "   - If all projects covered OR limits reached → **STOP - transition to TECHNICAL_QUESTION**\n\n"
        ...
        "     * Questions about topics the candidate has already answered\n"
        "   - If a project has been sufficiently discussed, move to a NEW project\n\n"

        "5. **If you can still ask (limits not reached)**:\n"
        "   - Select a unique question from EXAMPLE QUESTIONS that hasn't been asked\n"
        "   - Make sure it's relevant to the current project\n"
        "   - Set `go_to_next_step = false`\n\n"
        ...
        "```json\n"
        "{{\n"
        "  \"message\": \"Your project question or empty string if moving to next step\",\n"
        "  \"go_to_next_step\": true or false\n"
        "}}\n"
        "```"
    ),
...
ASK_TECHNICAL_PROMPT = ChatPromptTemplate.from_messages([
    (
        ...
        "   - **CRITICAL LIMIT: Maximum 7 technical questions total**\n"
        ...
        "   - If under 7 questions AND still have unexplored technical topics → Ask next relevant question from EXAMPLE QUESTIONS (that hasn't been asked)\n"
        ...
        "- You've reached 7 technical questions OR\n"
        "- Technical abilities have been sufficiently assessed (even if under 7 questions) OR\n"
        "- No more meaningful technical topics to explore in the resume or history\n"
        "- Set message = empty string, `go_to_next_step = true`\n\n"
...
ASK_BEHAVIORAL_PROMPT = ChatPromptTemplate.from_messages([
    (
        ...
        "**LIMIT:** Maximum 7 behavioral questions total.\n"
        "**FLEXIBILITY:** You don't need to ask all 7 questions if you've sufficiently assessed their behavioral competencies.\n\n"
        ...
        "   - **CRITICAL LIMIT: Maximum 7 behavioral questions total**\n"
        ...
        "3. **Decide what to ask**:\n"
        "   - **BEFORE selecting a question**: Cross-check against conversation history to ensure it hasn't been asked before\n"
        "   - If under 7 questions AND still have unexplored behavioral areas → Ask next relevant question from EXAMPLE QUESTIONS (that hasn't been asked)\n"
        "   - If reached 10 questions OR behavioral competencies sufficiently assessed → Proceed to WRAP_UP\n"
        "   - **REMEMBER**: Each question must be unique and not covered in previous conversation\n\n"
        
        "**When to move to next step:**\n"
        "- You've reached 7 behavioral questions OR\n"
        "- Behavioral competencies have been sufficiently assessed (even if under 7 questions) OR\n"
        ...
        "- Set message to empty string when transitioning: `go_to_next_step = true`\n\n"
        "**EXAMPLE QUESTIONS (USE VERBATIM - these are personalized behavioral questions for this candidate):**\n"
        "{example_questions_formatted}\n\n"
        
        "**Response format:**\n"
        "{{\n"
        "  \"message\": \"Your behavioral question or empty string if moving to next step\",\n"
        "  \"go_to_next_step\": true or false\n"
        "}}\n"
    ),
    (
